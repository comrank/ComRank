...
public class RealWorldHeterogeneousNetworkAutoInputForDiggOnly {
    ...
    private static SimpleWeightedGraph<String, DefaultEdge> createSimpleWeightedStringGraph() throws IOException
    {
    	...
    	//Digg
    	String inputFilePath = "E:\\TopKNodesIdentificationInHetergeneousNetworks\\DiggTwitterResults\\DiggUserReplyCommentStoryKTLE12-slpa.txt";
    	String inputFilePathOne = "E:\\TopKNodesIdentificationInHetergeneousNetworks\\DiggTwitterResults\\DiggUserReplyCommentStoryKTLE12-meta.txt";
		...
		try {
			...
			while((sOne = brOne.readLine()) != null) {
				if (sOne.contains("author = {")){
					...
					extractedAuthorNames = sOne.substring(10, sOne.length()-2);
				...
				if (sOne.contains("title = {")){
					...
					extractedPaperName = "paperrrrrr " + sOne.substring(9, sOne.length()-2).trim().replace(",","");
